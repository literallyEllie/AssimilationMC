...
public class AssiScoreboard {
    ...
    public void setup(Player player) {
        ...
        ModuleManager.getModuleManager().getPermissionManager().getGroups().forEach((s, assiPermGroup) -> {
            try {
                scoreboard.registerNewTeam(s).
                        setPrefix(Util.color(assiPermGroup.getPrefix().replace("%space%", " ")) + (assiPermGroup.getPrefix().length() == 2 ? "" : ChatColor.RESET));
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            if (p == player || p.getScoreboard() == null)
                continue;
