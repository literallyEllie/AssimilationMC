...
public class BungeeCordBuycraftPlatform implements IBuycraftPlatform {
    ...
    @Override
    public void callDonationEvent(QueuedPlayer player, QueuedCommand command, DonationUpdateType updateType) {
        ProxyServer.getInstance().getPluginManager().callEvent(new DonationUpdateEvent(player, command, updateType));
    }
