...
//        sanitize();
    ...
    private void sanitize() {
        if (to.contains(PARAM_SEPARATOR))
            to = to.replace(PARAM_SEPARATOR, "#####");

        if (from.contains(PARAM_SEPARATOR))
            from = from.replace(PARAM_SEPARATOR, "#####");

        if (subject.contains(PARAM_SEPARATOR))
            subject = subject.replace(PARAM_SEPARATOR, "####");

        for (int i = 0; i < args.length; i++) {
            String arg = args[i];
            if (arg.contains(PARAM_SEPARATOR))
                args[i] = arg.replace(PARAM_SEPARATOR, "####");
        }
